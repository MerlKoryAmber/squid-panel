...
echo "  - $SQUID_LISTEN, /etc/squid/spm-acl.conf, spm-peers.conf, spm-http_access.conf"
echo "  - /etc/squid/acl.d/"
echo "  - /etc/squid/*.keytab (panel copy, e.g. krb5.keytab)"
echo "  - ${SQUID_CONF}.spm-install-* and ${SQUID_CONF}.spm-listen-* / ${SQUID_CONF}.spm-policy-* backups"
...
echo "  - /etc/squid/spm-acl.conf / spm-peers.conf / spm-http_access.conf"
